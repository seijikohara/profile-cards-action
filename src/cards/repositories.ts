...
// Horizontal geometry: names on the left, bars emanating from a shared axis.
const LABEL_X = CARD_PADDING;
const BAR_START_X = CARD_PADDING + 258;
const VALUE_GAP = 8;
...
export function renderRepositories(data: ProfileData, theme: Theme, fontFaceCss: string): string {
  ...
  ranking.rows.forEach((row, index) => {
    ...
    labels.push(el('text', { x: LABEL_X, y: rowCenter + 4, class: 't-label' }, textNode(truncate(row.nameWithOwner))));
    bars.push(
      el(
        'g',
        {
  ...
  const height = bandBottom + CARD_PADDING;
  ...
  return cardFrame(
    ...
    el('g', { class: 'fade' }, axis, ...labels, ...values, empty),
    ...bars
  );
}
